...
class CuDeuron():
    ...
    CuDeuron is the neural network framework that includes and uses CUDA for learning with increasing performance speed:

    There are 5 activation functions available relu, leaky_relu, sigmoid, tanh, softmax; please check CuDeuron.ACTIVATION...
    ...
    class COST():
        """
        CuDeuron.COST.SQUARE_ERROR_COST

        CuDeuron.COST.CROSS_ENTROPY_COST

        CuDeuron.COST.EXTENDED_CROSS_ENTROPY_COST
        """
    ...
    class ACTIVATION():
        """
        CuDeuron.ACTIVATION.RELU

        CuDeuron.ACTIVATION.LEAKY_RELU

        CuDeuron.ACTIVATION.SIGMOID

        CuDeuron.ACTIVATION.TANH

        CuDeuron.ACTIVATION.SOFTMAX
        """
    ...
    class TRANSFORM():
        """
        CuDeuron.TRANSFORM.NORMALIZED

        CuDeuron.TRANSFORM.MEAN_NORMALIZED

        CuDeuron.TRANSFORM.STANDARDIZED

        CuDeuron.TRANSFORM.NO_TRANSFORM
        """
    ...
    class REGULARIZATION():
        """
        CuDeuron.REGULARIZATION.L1

        CuDeuron.REGULARIZATION.L2

        CuDeuron.REGULARIZATION.DROPOUT

        CuDeuron.REGULARIZATION.NO_REGULARIZATION
        """
    ...
    class OPTIMIZER():
        """
        CuDeuron.OPTIMIZER.MOMENTUM

        CuDeuron.OPTIMIZER.ADAM

        CuDeuron.OPTIMIZER.NO_OPTIMIZER
        """
    ...
    def __init__(self, X: np.array, Y: np.array, layer_list: list, cost_type: str, epoch_number: int, alpha: float):
        ...
        exp: [(100, CuDeuron.ACTIVATION.SIGMOID), (30, CuDeuron.ACTIVATION.TANH), (5, CuDeuron.ACTIVATION.SOFTMAX)]
        cost_type --> Specifies the type of cost that learning algorithm going to use, (check CuDeuron.ACTIVATION)
        ...
        if self.__prepare_data(X, Y) and self.__create_and_validate_layer_list(layer_list) and alpha > 0 and epoch_number > 0 and (cost_type in CuDeuron.COST.values):
            ...
            self.__regularization_param = CuDeuron.REGULARIZATION.NO_REGULARIZATION
            self.__transform_param_X = CuDeuron.TRANSFORM.NO_TRANSFORM
            self.__optimizer = CuDeuron.OPTIMIZER.NO_OPTIMIZER
            self.__transform_param_Y = CuDeuron.TRANSFORM.NO_TRANSFORM
            self.__lambd = 0
            self.__keep_prob = 0
            self.__batch_size = self.__m
            self.__batch_count = 1
            self.__seed = np.random.randint(1000000)
            cp.random.seed(self.__seed)
            self.__console_log = ""
    ...
    def __prepare_data(self, X, Y):
        ...
        # Initialize CUDA and turn numoy arrays to cupy
        with cp.cuda.Device(0):
            self.__X_raw = cp.asarray(X)
            self.__Y_raw = cp.asarray(Y)
    ...
    def __initialize_params(self):
        ...
        for i in range(1,self.__L):
            self.__W[str(i)] = cp.random.randn(self.__layer_number_of_nodes_list[i-1], self.__layer_number_of_nodes_list[i]) * cp.sqrt(2/(self.__layer_number_of_nodes_list[i-1]))
            self.__B[str(i)] = cp.zeros((self.__layer_number_of_nodes_list[i], 1))
    ...
    def __apply_mini_batch(self):
        permutation = list(cp.random.permutation(self.__m))
    ...
    def normalize(self, is_Y_transform = False):
        ...
        if is_Y_transform:
            self.__transform_param_Y = CuDeuron.TRANSFORM.NORMALIZED
            maxx = cp.max(self.__Y_raw, axis = 1, keepdims=True)
            minn = cp.min(self.__Y_raw, axis = 1, keepdims=True)
            self.__transform_data_Y = {"Max": maxx, "Min": minn}
        else:
            self.__transform_param_X = CuDeuron.TRANSFORM.NORMALIZED
            maxx = cp.max(self.__X_raw, axis = 1, keepdims=True)
            minn = cp.min(self.__X_raw, axis = 1, keepdims=True)
    ...
    def mean_normalize(self, is_Y_transform = False):
        ...
        if is_Y_transform:
            ...
            maxx = cp.max(self.__Y_raw, axis = 1, keepdims=True)
            minn = cp.min(self.__Y_raw, axis = 1, keepdims=True)
            meann = cp.mean(self.__Y_raw, axis = 1, keepdims=True)
            self.__transform_data_Y = {"Max": maxx, "Min": minn, "Mean": meann}
        else:
            self.__transform_param_X = CuDeuron.TRANSFORM.NORMALIZED
            maxx = cp.max(self.__X_raw, axis = 1, keepdims=True)
            minn = cp.min(self.__X_raw, axis = 1, keepdims=True)
            meann = cp.mean(self.__X_raw, axis = 1, keepdims=True)
    ...
    def standardize(self, is_Y_transform = False):
        ...
        if is_Y_transform:
            self.__transform_param_Y = CuDeuron.TRANSFORM.STANDARDIZED
            meann = cp.mean(self.__Y_raw, axis = 1, keepdims=True)
            stdd = cp.std(self.__Y_raw, axis = 1, keepdims=True)
            self.__transform_data_Y = {"Mean": meann, "Std": stdd}
        else:
            self.__transform_param_X = CuDeuron.TRANSFORM.STANDARDIZED
            meann = cp.mean(self.__X_raw, axis = 1, keepdims=True)
            stdd = cp.std(self.__X_raw, axis = 1, keepdims=True)
    ...
    def __reset_applied_transform(self):
        if self.__transform_param_X != CuDeuron.TRANSFORM.NO_TRANSFORM:
            self.__X_raw = cp.asarray(self.redo_transform(self.__X_raw))
        if self.__transform_param_Y != CuDeuron.TRANSFORM.NO_TRANSFORM:
            self.__Y_raw = cp.asarray(self.redo_transform(self.__Y_raw, True))

    def transform(self, data_np: np.array, is_Y_transform = False):
        """
        Transform target matrix by the rules of X or Y (by selection)

        data_np --> The target matrix to transform (numpy.array mandatory)
        ...
        if is_Y_transform:
            if self.__transform_param_Y == CuDeuron.TRANSFORM.NORMALIZED:
                maxx = self.__transform_data_Y["Max"]
                minn = self.__transform_data_Y["Min"]
                return cp.asnumpy((data - minn) / (maxx - minn))
            elif self.__transform_param_Y == CuDeuron.TRANSFORM.STANDARDIZED:
                meann = self.__transform_data_Y["Mean"]
                stdd = self.__transform_data_Y["Std"]
                return cp.asnumpy((data - meann) / stdd)
            elif self.__transform_param_Y == CuDeuron.TRANSFORM.MEAN_NORMALIZED:
                maxx = self.__transform_data_Y["Max"]
                minn = self.__transform_data_Y["Min"]
                meann = self.__transform_param_Y["Mean"]
                return cp.asnumpy((data - meann) / (maxx - minn))
            else:
    ...
    def redo_transform(self, data_np: np.array, is_Y_transform = False):
        """
        Redo the transform of the target matrix by the rules of X or Y (by selection)

        data_np --> The target matrix to redo its transformed form (numpy.array mandatory)
        is_Y_transform (Optional [Default: False]) --> if False apply transform rules of X, if True apply transform rules of Y
        """
        ...
        if is_Y_transform:
            if self.__transform_param_Y == CuDeuron.TRANSFORM.NORMALIZED:
                maxx = self.__transform_data_Y["Max"]
                minn = self.__transform_data_Y["Min"]
                return cp.asnumpy((data * (maxx - minn) + minn).get())
            elif self.__transform_param_Y == CuDeuron.TRANSFORM.STANDARDIZED:
                meann = self.__transform_data_Y["Mean"]
                stdd = self.__transform_data_Y["Std"]
                return cp.asnumpy((data * stdd + meann).get())
            elif self.__transform_param_Y == CuDeuron.TRANSFORM.MEAN_NORMALIZED:
                maxx = self.__transform_data_Y["Max"]
                minn = self.__transform_data_Y["Min"]
                meann = self.__transform_data_Y["Mean"]
                return cp.asnumpy((data * (maxx - minn) + meann).get())
            else:
                raise Exception("Error! Y value not transformed before")
        else:
            if self.__transform_param_X == CuDeuron.TRANSFORM.NORMALIZED:
    ...
    def set_regularization(self, regularization_param: str, regularization_factor: float = 0):
        ...
        regularization_param --> Specifies the type of regularization, please check CuDeuron.REGULARIZATION
        ...
        if regularization_param in CuDeuron.REGULARIZATION.values:
            if regularization_param == CuDeuron.REGULARIZATION.L1 or regularization_param == CuDeuron.REGULARIZATION.L2:
            ...
            elif regularization_param == CuDeuron.REGULARIZATION.DROPOUT:
                if regularization_factor >= 0 and regularization_factor < 1:
                    self.__regularization_param = CuDeuron.REGULARIZATION.DROPOUT
                    self.__lambd = 0
                    if regularization_factor == 0:
            ...
            raise Exception("Error! Invalid regularization parameter, (Please check CuDeuron.REGULARIZATION)")
    ...
    def set_optimizer(self, optimizer: str, beta1 = 0.9, beta2 = 0.99):
        ...
        optimizer --> Specifies the optimizer type, (Please check CuDeuron.OPTIMIZER...)
        ...
        if optimizer in CuDeuron.OPTIMIZER.values:
            if beta1 > 0 and beta2 > 0:
                if optimizer == CuDeuron.OPTIMIZER.NO_OPTIMIZER:
                    self.cancel_optimizer()
                elif optimizer == CuDeuron.OPTIMIZER.MOMENTUM:
            ...
            raise Exception("Error! Invalid optimizer parameter, please check (CuDeuron.OPTIMIZER...)")
    ...
    def __initialize_momentum_velocity(self):
        ...
        for i in range(1, self.__L):
            self.__velocity_data["dW"+str(i)] = cp.zeros(self.__W[str(i)].shape)
            self.__velocity_data["dB"+str(i)] = cp.zeros(self.__B[str(i)].shape)

    def __initialize_adam_optimizer(self):
        self.__initialize_momentum_velocity()
        self.__adam_data = dict()
        for i in range(1, self.__L):
            self.__adam_data["dW"+str(i)] = cp.zeros(self.__W[str(i)].shape)
            self.__adam_data["dB"+str(i)] = cp.zeros(self.__B[str(i)].shape)

    def __forward_prop(self):
        for i in range(1,self.__L):
            self.__Z[str(i)] = cp.dot(self.__W[str(i)].T, self.__A[str(i - 1)]) + self.__B[str(i)]
        ...
        if self.__regularization_param == self.REGULARIZATION.DROPOUT:
            for j in range(1,self.__L - 1):
                D = (cp.random.rand(self.__A[str(j)].shape[0], self.__A[str(j)].shape[1]) < self.__keep_prob).astype(int)
                self.__A[str(j)] *= D
    ...
    def __activation(self, z, type):
        if type == self.ACTIVATION.RELU:
            ...
            return cp.exp(z) / cp.sum(cp.exp(z), axis = 0, keepdims = True)
        else:
            raise Exception("Error!!! Types must be one of the CuDeuron.ACTIVATION... (RELU, SIGMOID, etc.)")
        return a

    def __cost(self):
        a = self.__A[str(self.__L - 1)]
        if self.__cost_type == self.COST.SQUARE_ERROR_COST:
            return ((1/(2*self.__m_current)) * cp.sum((self.__Y - a)**2)) + ((self.__lambd/(2*self.__m_current)) * self.__reg_value())
        elif self.__cost_type == self.COST.EXTENDED_CROSS_ENTROPY_COST:
            return (-1/self.__m_current) * cp.sum(self.__Y * cp.log(a) + (1 - self.__Y) * cp.log(1 - a)) + ((self.__lambd/(2*self.__m_current)) * self.__reg_value())
        elif self.__cost_type == self.COST.CROSS_ENTROPY_COST:
            return (-1/self.__m_current) * cp.sum(self.__Y * cp.log(a)) + ((self.__lambd/(2*self.__m_current)) * self.__reg_value())
    ...
    def __reg_value(self):
        ...
        if self.__regularization_param == self.REGULARIZATION.L1:
            for i in range(1,self.__L):
                reg += cp.sum(cp.abs(self.__W[str(i)]))
        elif self.__regularization_param == self.REGULARIZATION.L2:
            for i in range(1,self.__L):
                reg += cp.sum(self.__W[str(i)]**2)
        return reg
    ...
    def __backward_prop(self):
        ...
        for i in range(self.__L - 1, 0, -1):
            ...
            self.__dW[str(i)] = (1 / self.__m_current) * (cp.dot(self.__A[str(i - 1)], self.__dZ[str(i)].T))
            self.__backward_regularization(i)
            self.__dB[str(i)] = (1 / self.__m_current) * cp.sum(self.__dZ[str(i)],axis = 1, keepdims = True)
            if i != 1:
                self.__dA[str(i - 1)] = cp.dot(self.__W[str(i)], self.__dZ[str(i)])
                if self.__regularization_param == self.REGULARIZATION.DROPOUT:
    ...
    def __gradient_desc(self, iter_number):
        ...
        if self.__alpha_decay_rate != 0:
            alpha = (self.__alpha / (1 + self.__alpha_decay_rate * cp.floor(iter_number / self.__learning_decay_time_interval)))

        if self.__optimizer == CuDeuron.OPTIMIZER.NO_OPTIMIZER:
            for i in range(1,self.__L):
                self.__W[str(i)] -= alpha * self.__dW[str(i)]
                self.__B[str(i)] -= alpha * self.__dB[str(i)]
        elif self.__optimizer == CuDeuron.OPTIMIZER.ADAM:
            for i in range(1, self.__L):
    ...
    def __plot_cost(self):
        for i in range(len(self.__J)):
            self.__J[i] = cp.asnumpy(self.__J[i].get())
        plt.plot(self.__J, 'r-')
    ...
    def start(self):
        """
        The start function of CuDeuron learning algorithm. This function starts learning algorithm, prints and plots cost values of each 100 iteration (epoch). Returns learned weights as a tuple (W,B)
        """
        ...
        for iter in range(self.__epoch_number):
            ...
            if iter % 100 == 0 or iter == 0:
                ...
                print(self.__console_log)
            if iter % 20 == 0:
                self.__console_header = f"===== Current state:\t\t% {int((iter / self.__epoch_number) * 100)} {self.__console_pattern[self.__pattern_turn % 4]} =====\n\n"
                self.__console_log = self.__console_header+self.__console_content
                self.__pattern_turn += 1
                os.system('cls')
                clear_output(wait = True)
                print(self.__console_log)
        end_time = time.time()
        self.__reset_applied_transform()
        print(f"Iteration:\t{self.__epoch_number}\t\tCost:\t{self.__J[iter]}\tJ")
    ...
    def get_weights(self):
        ...
        CuDeuron.start() function must have been executed first
        """

        if self.__learn_state:
            return (self.__W, self.__B)
        else:
            raise Exception("Error! There are no learned weights yet, you must initialize CuDeuron.start() function first")
    ...
    def test(self, X: np.array):
        ...
        if self.__learn_state:
            ...
            if self.__transform_param_X != CuDeuron.TRANSFORM.NO_TRANSFORM:
                ...
                A_test["0"] = X
            A_test["0"] = cp.asarray(A_test["0"])

            for i in range(1,self.__L):
                Z_test[str(i)]= cp.dot(self.__W[str(i)].T, A_test[str(i - 1)]) + self.__B[str(i)]
                A_test[str(i)] = self.__activation(Z_test[str(i)], self.__layer_type_list[i])

            return cp.asnumpy(A_test[str(self.__L - 1)].get())
        else:
